...
@Command(name = "fetch-list", description = "Fetches a list by ID or name", mixinStandardHelpOptions = true)
public class FetchListCommand implements Runnable {
	...
	public void run() {
		try {
			...
			try {
				...
				response = client.getList(id);
			} catch (NumberFormatException ex) {
				response = client.getList(idOrName);
			}
